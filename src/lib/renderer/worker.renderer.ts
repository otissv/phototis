// KF-MIGRATE: Worker must receive/evaluate sampled Track outputs at time t; parameters here should not be static.
...
async function ensureHeavyInit(width: number, height: number): Promise<void> {
  ...
  const [hybridMod, pipelineMod] = await Promise.all([
    import("@/lib/renderer/hybrid.renderer"),
    import("@/lib/renderer/asynchronous-pipeline.renderer"),
  ])
